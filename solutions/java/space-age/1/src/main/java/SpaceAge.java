class SpaceAge {
    ...
    double onMercury() {
        return seconds / (EARTH_YEAR_SECONDS * MERCURY_ORBITAL_PERIOD);
    }

    double onVenus() {
        return seconds / (EARTH_YEAR_SECONDS * VENUS_ORBITAL_PERIOD);
    }

    double onMars() {
        return seconds / (EARTH_YEAR_SECONDS * MARS_ORBITAL_PERIOD);
    }

    double onJupiter() {
        return seconds / (EARTH_YEAR_SECONDS * JUPITER_ORBITAL_PERIOD);
    }

    double onSaturn() {
        return seconds / (EARTH_YEAR_SECONDS * SATURN_ORBITAL_PERIOD);
    }

    double onUranus() {
        return seconds / (EARTH_YEAR_SECONDS * URANUS_ORBITAL_PERIOD);
    }

    double onNeptune() {
        return seconds / (EARTH_YEAR_SECONDS * NEPTUNE_ORBITAL_PERIOD);
    }
}